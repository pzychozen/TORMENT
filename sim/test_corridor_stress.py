...
def main(args=None):
    ...
    steps = int(getattr(args, "steps", 200) or 200)
    print_every = int(getattr(args, "print_every", 10) or 10)

    for i in range(steps):
        text = random.choice(texts) + f" | step={i}"
        ...
        print(
            ...
            f"coh={coh:.6f} z={z:+.3f} "
            f"in_corr={in_corr:.1f} surv={surv:.3f} "
            f"tear={tear:.3f} phasev={phasev:.3f} "
            f"disp={disp:.3e} coh_phase={coh_phase:.6f}"
        )
