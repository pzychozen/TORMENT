...
class SubstrateTx:
    ...
    def __init__(self, connection: sqlite3.Connection, operation_id: bytes) -> None:
        ...
        self.legacy_relationship_admitted: list[tuple[bytes, bytes, int, bytes, bytes, bytes, bytes, bytes, str, str, tuple[tuple[int, str, str, bytes, bytes], ...]]] = []
        self.legacy_representation_admitted: list[tuple[bytes, bytes, bytes, bytes, bytes, bytes, bytes, bytes, bytes, bytes, int, str, int]] = []
    ...
    def validate(self) -> None:
        ...
        for relationship_id,revision_id,ordinal,admission_record_id,transition_id,snapshot_id,artifact_id,artifact_record_id,alias_kind,alias_value,endpoints in self.legacy_relationship_admitted:
            transition = self.execute("SELECT operation_id,transition_kind,origin_kind FROM semantic_transitions WHERE transition_id=?",(transition_id,)).fetchone()
            if transition != (self.operation_id,"LEGACY_RELATIONSHIP_ADMISSION","LEGACY_ADMISSION"): raise SubstrateInvariantViolation("H7 imported relationship is not published through a legacy admission transition")
            if self.execute("SELECT creating_transition_id,current_revision_id,current_revision_ordinal FROM relationships WHERE relationship_id=?",(relationship_id,)).fetchone() != (transition_id,revision_id,ordinal): raise SubstrateInvariantViolation("H7 admitted relationship current publication is incomplete")
            if self.execute("SELECT lineage_kind,predecessor_revision_id,predecessor_revision_ordinal FROM relationship_revisions WHERE relationship_id=? AND relationship_revision_id=? AND revision_ordinal=?",(relationship_id,revision_id,ordinal)).fetchone() != ("LEGACY_PREDECESSOR_UNKNOWN",None,None): raise SubstrateInvariantViolation("H7 imported relationship lineage is not legacy predecessor unknown")
            if self.execute("SELECT 1 FROM relationship_revision_effects WHERE transition_id=? AND relationship_id=? AND relationship_revision_id=? AND relationship_revision_ordinal=?",(transition_id,relationship_id,revision_id,ordinal)).fetchone() is None: raise SubstrateInvariantViolation("H2 legacy admission relationship revision effect is missing")
            if self.execute("SELECT 1 FROM legacy_admission_effects WHERE transition_id=? AND admission_record_id=?",(transition_id,admission_record_id)).fetchone() is None: raise SubstrateInvariantViolation("H2 legacy relationship admission effect is missing")
            if self.execute("SELECT 1 FROM legacy_admission_records r JOIN legacy_admission_batches b USING(admission_batch_id) JOIN legacy_artifact_records ar USING(legacy_artifact_record_id) WHERE r.admission_record_id=? AND r.admission_status='ADMITTED' AND b.legacy_snapshot_id=? AND ar.legacy_artifact_id=? AND ar.legacy_artifact_record_id=?",(admission_record_id,snapshot_id,artifact_id,artifact_record_id)).fetchone() is None: raise SubstrateInvariantViolation("H7 relationship admission record evidence linkage is incomplete")
            if self.execute("SELECT 1 FROM legacy_relationship_aliases WHERE legacy_source_namespace_id=(SELECT legacy_source_namespace_id FROM legacy_snapshots WHERE legacy_snapshot_id=?) AND alias_kind=? AND alias_value=? AND relationship_id=?",(snapshot_id,alias_kind,alias_value,relationship_id)).fetchone() is None: raise SubstrateInvariantViolation("H8 legacy relationship alias does not match admitted relationship")
            if self.execute("SELECT 1 FROM operation_outputs o JOIN relationship_revision_effects e ON e.transition_id=? WHERE o.operation_id=? AND o.output_kind='RELATIONSHIP' AND o.output_role='LEGACY_RELATIONSHIP_ADMISSION' AND o.relationship_id=? AND o.relationship_revision_id=? AND o.relationship_revision_ordinal=? AND e.relationship_id=o.relationship_id AND e.relationship_revision_id=o.relationship_revision_id AND e.relationship_revision_ordinal=o.relationship_revision_ordinal",(transition_id,self.operation_id,relationship_id,revision_id,ordinal)).fetchone() is None: raise SubstrateInvariantViolation("H8 legacy relationship admission output does not match publication")
            for endpoint_ordinal,endpoint_role,alias_value,object_id,scope_id in endpoints:
                if self.execute("SELECT 1 FROM relationship_revision_endpoints WHERE relationship_revision_id=? AND endpoint_ordinal=? AND endpoint_role=? AND endpoint_semantic_scope_id=? AND object_id=? AND binding_mode='IDENTITY' AND bound_object_revision_id IS NULL AND bound_object_revision_ordinal IS NULL",(revision_id,endpoint_ordinal,endpoint_role,scope_id,object_id)).fetchone() is None: raise SubstrateInvariantViolation("H7 legacy relationship endpoint aggregate is incomplete")
                if self.execute("SELECT 1 FROM legacy_object_aliases WHERE legacy_source_namespace_id=(SELECT legacy_source_namespace_id FROM legacy_snapshots WHERE legacy_snapshot_id=?) AND alias_kind='EID' AND alias_value=? AND object_id=?",(snapshot_id,alias_value,object_id)).fetchone() is None: raise SubstrateInvariantViolation("H7 legacy relationship endpoint alias did not resolve inside the source namespace")
        for representation_id,admission_record_id,transition_id,snapshot_id,node_artifact_id,map_artifact_id,shard_artifact_id,map_artifact_record_id,object_id,revision_id,revision_ordinal,alias_value,payload_length in self.legacy_representation_admitted:
            transition = self.execute("SELECT operation_id,transition_kind,origin_kind FROM semantic_transitions WHERE transition_id=?",(transition_id,)).fetchone()
            if transition != (self.operation_id,"LEGACY_REPRESENTATION_ADMISSION","LEGACY_ADMISSION"): raise SubstrateInvariantViolation("H7 imported representation is not published through a legacy admission transition")
            representation = self.execute("SELECT source_kind,source_object_id,source_object_revision_id,source_object_revision_ordinal,representation_class,generation,derivation_contract_version,encoding_id,expected_payload_byte_length FROM representations WHERE representation_id=?",(representation_id,)).fetchone()
            if representation is None or representation[0:4] != ("OBJECT_REVISION",object_id,revision_id,revision_ordinal) or representation[4] != "LEGACY_EMBEDDING_CAPTURE" or representation[5] != 1 or not representation[6] or not representation[7] or representation[8] != payload_length: raise SubstrateInvariantViolation("H7 legacy representation identity or exact source revision linkage is incomplete")
            if self.execute("SELECT lineage_kind FROM object_revisions WHERE object_id=? AND object_revision_id=? AND revision_ordinal=?",(object_id,revision_id,revision_ordinal)).fetchone() != ("LEGACY_PREDECESSOR_UNKNOWN",): raise SubstrateInvariantViolation("H7 legacy representation source is not the imported object revision")
            if self.execute("SELECT 1 FROM legacy_object_aliases WHERE legacy_source_namespace_id=(SELECT legacy_source_namespace_id FROM legacy_snapshots WHERE legacy_snapshot_id=?) AND alias_kind='EID' AND alias_value=? AND object_id=?",(snapshot_id,alias_value,object_id)).fetchone() is None: raise SubstrateInvariantViolation("H7 legacy representation source alias did not resolve inside the source namespace")
            if self.execute("SELECT readiness,operational_disposition,selected_integrity_measurement_id FROM representation_current_state WHERE representation_id=?",(representation_id,)).fetchone() != ("UNKNOWN","RECONCILIATION_REQUIRED",None): raise SubstrateInvariantViolation("H7 legacy representation is falsely ready or usable")
            payload = self.execute("SELECT payload_bytes,observed_payload_byte_length FROM representation_payloads WHERE representation_id=?",(representation_id,)).fetchone()
            if payload is None or len(payload[0]) != payload_length or payload[1] != payload_length: raise SubstrateInvariantViolation("H7 legacy representation payload is incomplete")
            if self.execute("SELECT 1 FROM integrity_expectations WHERE subject_kind='REPRESENTATION' AND representation_id=?",(representation_id,)).fetchone() is not None: raise SubstrateInvariantViolation("H7 legacy representation self-certifies captured payload")
            if self.execute("SELECT 1 FROM representation_state_effects WHERE transition_id=? AND representation_id=? AND readiness='UNKNOWN' AND operational_disposition='RECONCILIATION_REQUIRED' AND selected_measurement_id IS NULL",(transition_id,representation_id)).fetchone() is None: raise SubstrateInvariantViolation("H2 legacy representation state effect is missing")
            if self.execute("SELECT 1 FROM legacy_admission_effects WHERE transition_id=? AND admission_record_id=?",(transition_id,admission_record_id)).fetchone() is None: raise SubstrateInvariantViolation("H2 legacy representation admission effect is missing")
            if self.execute("SELECT 1 FROM legacy_admission_records r JOIN legacy_admission_batches b USING(admission_batch_id) JOIN legacy_artifact_records ar USING(legacy_artifact_record_id) WHERE r.admission_record_id=? AND r.admission_status='ADMITTED' AND b.legacy_snapshot_id=? AND ar.legacy_artifact_id=? AND ar.legacy_artifact_record_id=?",(admission_record_id,snapshot_id,map_artifact_id,map_artifact_record_id)).fetchone() is None: raise SubstrateInvariantViolation("H7 legacy representation admission evidence linkage is incomplete")
            if self.execute("SELECT count(*) FROM legacy_artifacts WHERE legacy_snapshot_id=? AND legacy_artifact_id IN (?,?,?)",(snapshot_id,node_artifact_id,map_artifact_id,shard_artifact_id)).fetchone() != (3,): raise SubstrateInvariantViolation("H7 legacy representation artifact chain is incomplete")
            if self.execute("SELECT 1 FROM operation_outputs o JOIN representation_state_effects e ON e.transition_id=? WHERE o.operation_id=? AND o.output_kind='REPRESENTATION' AND o.output_role='LEGACY_REPRESENTATION_ADMISSION' AND o.representation_id=? AND e.representation_id=o.representation_id AND e.readiness='UNKNOWN' AND e.operational_disposition='RECONCILIATION_REQUIRED'",(transition_id,self.operation_id,representation_id)).fetchone() is None: raise SubstrateInvariantViolation("H8 legacy representation admission output does not match publication")
