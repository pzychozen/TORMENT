...
- **HEAD:** `2c85b2e` / `origin/main` / clean.
...
- **Active next:** Hilmir has **selected the real-cognition direction** (priority order: (1) real cognition / meaningful Envelope Audit / dreaming; (2) participation guidance v2 later; (3) stronger anti-control locks later; (4) database/substrate last — already mapped/ratified, rechecked later). The **first doorway is FILED docs-only**: the **model-API truthfulness/evidence audit boundary frame** (`444cc9b`) — a bounded, **default-off, observe-and-surface-only**, ephemeral, **structurally non-reentrant, no-feedback** audit that observes evidence relation and reports coarse flags **which no runtime path honors**. **§9 operator confirmation is now given (Hilmir); the first step past the doorway is a docs-only non-reentry constraints lock (`bb9bb16`) — a pre-implementation constraints frame that authorizes no implementation/tests/schema/runtime/model-call/provider/endpoint/memory-write/output-control/substrate. Anything further is gated by that frame's closing gate: Hilmir explicit next-step confirmation + a Codex non-reentry challenge.** No model-API runtime / implementation / integration is opened; **dream remains downstream and separately gated** (Regime-B, not part of this doorway). The prior **A0 defer still stands** for the pre-substrate content-free shell (duplicates ReflectionTrace / `ReviewResult`, or risks the B-O6.1 anti-agreeableness failure); the model-API audit track is the chosen path to give real Envelope-Audit value. Current posture: **context-extractor slice CLOSED-FOR-NOW** (`2c85b2e`, `11 passed`). The lane now has **two proven pure stages, both called nowhere**: source-selection (`AssembledContext` selected `blocks` → candidate `admitted_context_items`) and packet minimization (candidate `admitted_context_items` → admissible evidence packet). **Load-bearing next gate:** real `ContextBlock` dicts carry sensitivity/source markers in **`metadata`, not at the top level the packet builder currently reads** — so the marker filter would not bite as-is. **Next real fork: a marker-lift / bounded `metadata`-read proposal** (lift markers to the top level, or teach the builder to read them) **before any production wiring**. Still pending after that: a **same-turn prompt-inclusion proof** and an **output-sink / wiring decision**. **Production wiring remains unopened**; **evaluator / model call / provider / prompt / endpoint remain unopened**; **dream / private cognition, Gate D, Envelope Audit implementation, R-surface, participation v2, writer authority, database/substrate remain deferred.** Anchors: boundary frame `444cc9b` (`docs/TORMENT_MODEL_API_TRUTHFULNESS_EVIDENCE_AUDIT_BOUNDARY_FRAME_v0.1.md`); Document B §7/§10 (B-O6/B-O6.1/B-O8); P4 §9/§10; Ledger Observational-Boundary §3; Track-A truthfulness envelope; MCP capability boundary. Claude proposes from source; GPT steers; Codex challenges boundary-bearing choices; Hilmir resolves true operator forks.
