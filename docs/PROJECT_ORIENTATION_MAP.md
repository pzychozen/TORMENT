...
- **HEAD:** `4902586` / `origin/main` / clean.
...
- **Active next:** Strategic direction unchanged (Hilmir-selected): **real cognition / meaningful Envelope Audit is the active doorway → later dreaming / private cognition → later database/substrate** (participation guidance v2 and stronger anti-control locks remain later items). **The evidence-packet filtering boundary is now substantially closed for the current pure, called-nowhere stage** — four proven pure stages exist: (1) selected-item extractor (`2c85b2e`), (2) packet minimization / caps (`4dd6ff6`), (3) bounded metadata-marker read (`0f86fbb`), and (4) identity-context structural exclusion (contract §4A, `4902586`). **The next forward gate is NOT another broad contract doc** unless source proves one is required. It is a **source-grounded same-turn prompt-inclusion proof / characterization**: establish, tests-only and with **no production wiring**, whether a future audit packet can be sourced from evidence that was *actually included in the same turn's model-visible prompt* — **not** raw hits, **not** fresh retrieval, **not** re-filtered / audit-expanded context (contract §2/§5). **After** that proof, the next gate is an **output-sink / wiring decision**. Bias: **tests / characterization over new docs**; no new contract doc unless an actual contradiction is found. **Production wiring remains unopened**; **evaluator / model call / provider / prompt / endpoint remain unopened**; **dream / private cognition, Gate D, Envelope Audit implementation, R-surface, participation v2, writer authority, database/substrate remain deferred.** Anchors: contract `docs/TORMENT_MODEL_API_TRUTHFULNESS_AUDIT_ADMISSIBLE_EVIDENCE_PACKET_CONTRACT_v0.1.md` (§2/§5 same-turn rule, §4 + §4A exclusions); boundary frame `444cc9b` (`docs/TORMENT_MODEL_API_TRUTHFULNESS_EVIDENCE_AUDIT_BOUNDARY_FRAME_v0.1.md`); Document B §7/§10 (B-O6/B-O6.1/B-O8); P4 §9/§10; Ledger Observational-Boundary §3; Track-A truthfulness envelope; MCP capability boundary. Claude proposes from source; GPT steers; Codex challenges boundary-bearing choices; Hilmir resolves true operator forks.
