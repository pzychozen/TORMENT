...
- **HEAD:** `9e0cbca` / `origin/main` / clean.
...
- **Active next:** participation guidance v1 is **closed-for-now** (implemented + measured; advisory-only). Next is **unselected and now heavier** — every remaining capability is operator/Codex-gated: (1) participation guidance **v2 consumer** — let the candidate gently shape response assembly (brevity/restraint cue, never suppress / `None`) — output-adjacent, heaviest gate; (2) next cognition layer (Document B / Layer-1 ephemeral private thinking + Envelope Audit, the roadmap's Gate D) — highest living-system value, big gate; (3) R-field (unblocked, but R = `strength` at ingest / only lives on retrieval — murky, lower value). The only Codex-light remaining slice is observability (e.g. `query()` explain parity with `trace()`), telemetry not capability. The geometric / ephemeral MemoryPlan-shaping vein is harvested. **A fresh-chat handoff is recommended before opening any of these** — §0 + the frame doc carry the state. Claude proposes from source; GPT steers; Codex challenges boundary-bearing choices; Hilmir resolves true operator forks.
