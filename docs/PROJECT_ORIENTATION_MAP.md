...
- **HEAD:** `2496867` / `origin/main` / clean.
...
- **Active next:** Strategic direction unchanged (Hilmir-selected): **real cognition / meaningful Envelope Audit is the active doorway → later dreaming / private cognition → later database/substrate** (participation guidance v2 and stronger anti-control locks remain later items). The **marker-lift / bounded `metadata`-read proposal gate is now CLOSED by `0f86fbb`** — the pure packet builder reads allowlisted exclusion markers from `item["metadata"]` (read-only, discarded, no output/projection change, no marker-lift helper, no production caller). The lane still has **two proven pure stages, both called nowhere**: source-selection (`AssembledContext` selected `blocks` → candidate `admitted_context_items`) and packet minimization with metadata-aware exclusion (candidate `admitted_context_items` → admissible evidence packet). **The next true gate is NOT production wiring.** It is the **separately ratified identity-block categorical exclusion decision**, which must be proposed → Codex-challenged → ratified **before** any production wiring: `block_type=="identity_context"` / identity-block categorical exclusion was **deliberately not implemented** (Codex chose **Option C** for the metadata-read slice). **Why it is load-bearing:** the assembler may **drop raw-hit markers** (`canon`, `tier`, `governance`, `scope`, `srg`, `deep_memory`) at conversion, leaving identity/canon core hits such as `seed_canon`, `identity_anchor`, or `drift_correction` **marker-invisible after assembly**; the only reliable post-assembler signal may be `block_type=="identity_context"`, but honoring it **expands beyond marker-read into source-surface category logic** and therefore requires its own proposal/challenge/ratification. Still pending after that gate: a **same-turn prompt-inclusion proof** and an **output-sink / wiring decision**. **Production wiring remains unopened**; **evaluator / model call / provider / prompt / endpoint remain unopened**; **dream / private cognition, Gate D, Envelope Audit implementation, R-surface, participation v2, writer authority, database/substrate remain deferred.** Anchors: contract `docs/TORMENT_MODEL_API_TRUTHFULNESS_AUDIT_ADMISSIBLE_EVIDENCE_PACKET_CONTRACT_v0.1.md`; boundary frame `444cc9b` (`docs/TORMENT_MODEL_API_TRUTHFULNESS_EVIDENCE_AUDIT_BOUNDARY_FRAME_v0.1.md`); Document B §7/§10 (B-O6/B-O6.1/B-O8); P4 §9/§10; Ledger Observational-Boundary §3; Track-A truthfulness envelope; MCP capability boundary. Claude proposes from source; GPT steers; Codex challenges boundary-bearing choices; Hilmir resolves true operator forks.
