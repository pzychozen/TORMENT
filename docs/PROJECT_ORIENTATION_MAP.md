...
- **HEAD:** `af98662` / `origin/main` / clean.
...
- **Active next:** Strategic direction unchanged (Hilmir-selected): **real cognition / meaningful Envelope Audit is the active doorway → later dreaming / private cognition → later database/substrate** (participation guidance v2 and stronger anti-control locks remain later items). **The pure audit-evidence path has now proven the packet filters, the assembler same-turn prompt-context invariant, AND the output-sink co-occurrence gap** — six pure called-nowhere / tests-only stages: (1) selected-item extractor (`2c85b2e`), (2) packet minimization / caps (`4dd6ff6`), (3) bounded metadata-marker read (`0f86fbb`), (4) identity-context structural exclusion (contract §4A, `4902586`), (5) assembler prompt-context invariant (`a695a85`), and (6) output-sink co-occurrence gap (`af98662`). **The next gate is a true implementation fork — do NOT auto-open live wiring:** **(A)** thread `AssembledContext` / selected admitted context into `AgentRunner` / `TurnResult` as observation-only input; **(B)** caller-supplied-`response_text` audit tooling around `/retrieve`-style context; **(C)** a **pure, called-nowhere candidate sidecar builder** that accepts both halves explicitly (`response_text` + `AssembledContext` / selected items → audit sidecar), still **no sink selected**; **(D)** pause for Codex/operator ratification if any option changes authority boundaries. **Any next implementation that chooses or creates a sink requires Codex review first.** Likely recommendation: **C** (moves code forward without selecting live output wiring). **Production wiring remains unopened until explicitly ratified.** Still parked: **evaluator, model call, provider / prompt, endpoint behavior, persistence, output control, memory write, database / substrate, dream / private cognition, Gate D / Envelope Audit implementation, R-surface, participation v2, writer authority.** Anchors: contract `docs/TORMENT_MODEL_API_TRUTHFULNESS_AUDIT_ADMISSIBLE_EVIDENCE_PACKET_CONTRACT_v0.1.md` (§2/§5 same-turn rule, §4 + §4A exclusions); boundary frame `444cc9b` (`docs/TORMENT_MODEL_API_TRUTHFULNESS_EVIDENCE_AUDIT_BOUNDARY_FRAME_v0.1.md`); Document B §7/§10 (B-O6/B-O6.1/B-O8); P4 §9/§10; Ledger Observational-Boundary §3; Track-A truthfulness envelope; MCP capability boundary. Claude proposes from source; GPT steers; Codex challenges boundary-bearing choices; Hilmir resolves true operator forks.
