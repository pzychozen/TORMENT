...
- **HEAD:** `f83dc00` / `origin/main` / clean.
...
- **Active next:** Strategic direction unchanged (Hilmir-selected): **real cognition / meaningful Envelope Audit is the active doorway → later dreaming / private cognition → later database/substrate** (participation guidance v2 and stronger anti-control locks remain later items). **Current step (this docs-only slice): the identity-block categorical exclusion is now ratified at contract level.** The Admissible Evidence Packet Contract gains **§4A**, which admits `block_type=="identity_context"` (the value of `retrieval_assembler.BLOCK_IDENTITY`) as a **coarse structural exclusion for audit-packet filtering only**, recovering §4's seed/canon/identity categorical exclusion after `_hit_to_block(...)` drops raw markers (`canon`, `tier`/`character_tier`, `scope`, `governance`, `srg`, `deep_memory`) at assembly — so identity/canon core hits such as `seed_canon`, `identity_anchor`, or `drift_correction` are no longer marker-invisible. §4A is **category-not-authority** (not lifecycle/governance/identity truth; no new memory-row or sensitivity schema), **read-only / exclusion-only / never projected, persisted, or fed back**, raises §6 insufficient-admissible-evidence behavior without implying any unsupported claim, records the coupling to `BLOCK_IDENTITY` + `_classify_core_hit(...)`, and requires audit-boundary re-review if identity-classification semantics change. Codex **PASS WITH CONDITIONS** — applied in this docs slice. **This amendment authorizes only the next gate (still requiring review/authorization): a small pure builder/test slice** — add `_EXCLUDED_BLOCK_TYPES = ("identity_context",)` to `torment_service/audit_evidence_packet.py`; **no production import of `retrieval_assembler`**; a **test-only drift-pin** asserting the literal equals `retrieval_assembler.BLOCK_IDENTITY`; output projection unchanged; no production caller; **no wiring**. Tests live in `tests/test_audit_evidence_packet_builder.py` (+ one integration-style test in `tests/test_audit_evidence_context_selection.py`); no assembler/extractor/lifecycle change is required by source. Still pending after that gate: a **same-turn prompt-inclusion proof** and an **output-sink / wiring decision**. **Production wiring remains unopened**; **evaluator / model call / provider / prompt / endpoint remain unopened**; **dream / private cognition, Gate D, Envelope Audit implementation, R-surface, participation v2, writer authority, database/substrate remain deferred.** Anchors: contract `docs/TORMENT_MODEL_API_TRUTHFULNESS_AUDIT_ADMISSIBLE_EVIDENCE_PACKET_CONTRACT_v0.1.md` (§4 + new §4A); boundary frame `444cc9b` (`docs/TORMENT_MODEL_API_TRUTHFULNESS_EVIDENCE_AUDIT_BOUNDARY_FRAME_v0.1.md`); Document B §7/§10 (B-O6/B-O6.1/B-O8); P4 §9/§10; Ledger Observational-Boundary §3; Track-A truthfulness envelope; MCP capability boundary. Claude proposes from source; GPT steers; Codex challenges boundary-bearing choices; Hilmir resolves true operator forks.
