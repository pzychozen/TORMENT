...
- **HEAD:** `dcedfc0` / `origin/main` / clean.
...
- **Active next:** Strategic direction unchanged (Hilmir-selected): **real cognition / meaningful Envelope Audit is the active doorway → later dreaming / private cognition → later database/substrate** (participation guidance v2 and stronger anti-control locks remain later items). **The audit packet sink now exists observation-only on `TurnResult` (`e37da83`):** `AgentRunner.run_turn` builds `TurnResult.audit_evidence_packet` from explicit inputs only — final reviewed `response_text` + caller-supplied `audit_admitted_context_items` via the item-core builder — fail-soft, returned only on `TurnResult`, with **no same-turn provenance claim** and no routing into prompt/review/output/ingest/fabric/model-visible/persistence/authority. Nine stages now closed: extractor (`2c85b2e`), minimization/caps (`4dd6ff6`), metadata-marker read (`0f86fbb`), identity-context exclusion §4A (`4902586`), assembler prompt-context invariant (`a695a85`), output-sink co-occurrence gap (`af98662`), pure composition sidecar (`5f98fb1`), TurnResult staging seam (`dd052a3`), and TurnResult packet observation sink (`e37da83`). A tests-only / source-only **caller inventory (`c67b443`) found NO honest live caller path exists yet** — the only non-test `run_turn` callers are the runner-owner self-call and the demo, neither supplying `audit_admitted_context_items`; no endpoint or production-service caller exists. **B is the chosen direction (Codex/operator: PASS for B with conditions): a caller-owned same-turn provenance contract for an orchestration layer outside `AgentRunner` is now FILED docs-only** (`docs/TORMENT_AUDIT_CALLER_OWNED_SAME_TURN_PROVENANCE_CONTRACT_v0.1.md`): `AgentRunner` composes explicit inputs only and proves no provenance; the caller owns the same-turn claim and supplies **pre-extracted admitted item dicts (never `AssembledContext`)**; structural co-location on `TurnResult` does not prove provenance; the packet is observation-only and its absence is not a dishonesty / unsupportedness / suppression / retrieval / authority / memory signal; **no public endpoint or API, no verification flag, no wiring**. **The next gate is the concrete owner/path selection for caller-owned provenance — still Codex/operator-gated; wiring remains BLOCKED until a concrete owner/path is selected.** The first permissible later slice, *if* an owner is selected, is a **non-endpoint, tests-backed orchestration adapter/harness** that already owns **both halves in one call frame** and passes **only selected admitted item dicts** into `run_turn`, adding no public schema / verification flag / endpoint / persistence / model-provider-prompt-evaluator / output-control / authority; if no such owner exists, the next step is **another inventory/design fork, not wiring**. Staging, packet construction, and this contract do **not** authorize endpoint wiring, `/retrieve` or `/agent/query` changes, external/public API contracts, automatic provenance claims, a `same_turn_verified`/truth/authority flag, evaluator / model / provider / prompt work, persistence, output control, memory writes, database/substrate, dream/private cognition, Gate D, or Envelope Audit runtime. **The open question stays: who is allowed to supply `audit_admitted_context_items` honestly, and from which live path?** Any step answering it requires another Codex review first. Bias remains **forward code movement**, but **no hidden authority and no accidental provenance/sink expansion**. **Production wiring remains unopened until explicitly ratified.** Still parked: **evaluator, model call, provider / prompt, endpoint behavior, persistence, output control, memory write, database / substrate, dream / private cognition, Gate D / Envelope Audit implementation, R-surface, participation v2, writer authority.** Anchors: contract `docs/TORMENT_MODEL_API_TRUTHFULNESS_AUDIT_ADMISSIBLE_EVIDENCE_PACKET_CONTRACT_v0.1.md` (§2/§5 same-turn rule, §4 + §4A exclusions); boundary frame `444cc9b` (`docs/TORMENT_MODEL_API_TRUTHFULNESS_EVIDENCE_AUDIT_BOUNDARY_FRAME_v0.1.md`); Document B §7/§10 (B-O6/B-O6.1/B-O8); P4 §9/§10; Ledger Observational-Boundary §3; Track-A truthfulness envelope; MCP capability boundary. Claude proposes from source; GPT steers; Codex challenges boundary-bearing choices; Hilmir resolves true operator forks.
