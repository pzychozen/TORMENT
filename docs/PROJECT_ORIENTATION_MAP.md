...
- **HEAD:** `4dd6ff6` / `origin/main` / clean.
...
- **Active next:** Hilmir has **selected the real-cognition direction** (priority order: (1) real cognition / meaningful Envelope Audit / dreaming; (2) participation guidance v2 later; (3) stronger anti-control locks later; (4) database/substrate last — already mapped/ratified, rechecked later). The **first doorway is FILED docs-only**: the **model-API truthfulness/evidence audit boundary frame** (`444cc9b`) — a bounded, **default-off, observe-and-surface-only**, ephemeral, **structurally non-reentrant, no-feedback** audit that observes evidence relation and reports coarse flags **which no runtime path honors**. **§9 operator confirmation is now given (Hilmir); the first step past the doorway is a docs-only non-reentry constraints lock (`bb9bb16`) — a pre-implementation constraints frame that authorizes no implementation/tests/schema/runtime/model-call/provider/endpoint/memory-write/output-control/substrate. Anything further is gated by that frame's closing gate: Hilmir explicit next-step confirmation + a Codex non-reentry challenge.** No model-API runtime / implementation / integration is opened; **dream remains downstream and separately gated** (Regime-B, not part of this doorway). The prior **A0 defer still stands** for the pre-substrate content-free shell (duplicates ReflectionTrace / `ReviewResult`, or risks the B-O6.1 anti-agreeableness failure); the model-API audit track is the chosen path to give real Envelope-Audit value. Current posture: **pure evidence-packet builder slice CLOSED-FOR-NOW** (`4dd6ff6`, `23 passed`). The C2 contract landed (`b905165`), the exclusion markers were characterized (`fc2be5f`, `17 passed`), and the builder is a **pure, marker-filtered, capped, primitive-only transform — called nowhere in production** (production behavior unchanged). It implements C2 packet minimization **for explicit caller-supplied inputs only**; it does **not** prove the inputs were genuinely admitted. **Next real fork: the source-of-context wiring proposal** — *who supplies `admitted_context_items`, and how do we prove they were actually admitted into the response context for that turn?* The **evaluator / model call / provider / prompt / endpoint remain unopened**; **production wiring remains unopened**; the **source-of-context guarantee remains a later wiring gate**. The lower-priority forks (participation v2; stronger anti-control locks; database/substrate) and **dream / private cognition, Gate D, Envelope Audit implementation, R-surface, writer authority** remain unselected / deferred. Anchors: boundary frame `444cc9b` (`docs/TORMENT_MODEL_API_TRUTHFULNESS_EVIDENCE_AUDIT_BOUNDARY_FRAME_v0.1.md`); Document B §7/§10 (B-O6/B-O6.1/B-O8); P4 §9/§10; Ledger Observational-Boundary §3; Track-A truthfulness envelope; MCP capability boundary. Claude proposes from source; GPT steers; Codex challenges boundary-bearing choices; Hilmir resolves true operator forks.
