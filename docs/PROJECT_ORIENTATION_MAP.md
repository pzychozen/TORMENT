...
- **HEAD:** `dd052a3` / `origin/main` / clean.
...
- **Active next:** Strategic direction unchanged (Hilmir-selected): **real cognition / meaningful Envelope Audit is the active doorway → later dreaming / private cognition → later database/substrate** (participation guidance v2 and stronger anti-control locks remain later items). **The pure audit-evidence path is composed AND the observation staging seam now exists: `response_text` + caller-supplied candidate admitted context can coexist on `TurnResult` (`dd052a3`), and `response_text` + admitted context → existing admissible evidence packet via the pure sidecar (`5f98fb1`) — but NOTHING is wired: no packet is built or attached, and no sink is selected.** Eight pure / staging-only stages: (1) selected-item extractor (`2c85b2e`), (2) packet minimization / caps (`4dd6ff6`), (3) bounded metadata-marker read (`0f86fbb`), (4) identity-context structural exclusion (contract §4A, `4902586`), (5) assembler prompt-context invariant (`a695a85`), (6) output-sink co-occurrence gap (`af98662`), (7) pure composition sidecar (`5f98fb1`), and (8) AgentRunner/TurnResult observation staging seam (`dd052a3`, staging only — no packet/sink). **The "can both halves be composed?" and "can both halves coexist on a turn?" gates are CLOSED.** **The next gate is the packet-build + sink decision — Codex/operator-gated, do NOT auto-open.** Staging candidate admitted context on `TurnResult` does **not** authorize calling the sidecar builder, attaching an audit packet, selecting a sink, asserting same-turn verification, wiring endpoints, changing `/retrieve` or `/agent/query`, or opening evaluator / model / provider / prompt / persistence / output-control / memory-write / database / substrate / dream / private-cognition / Gate D / Envelope Audit. **Any step that builds/attaches a packet or imports/calls the sidecar from `AgentRunner`, `TurnResult`, `/retrieve`, an endpoint, or any output sink requires another Codex review first.** Bias remains **forward code movement**, but **no hidden authority and no accidental sink selection**. **Production wiring remains unopened until explicitly ratified.** Still parked: **evaluator, model call, provider / prompt, endpoint behavior, persistence, output control, memory write, database / substrate, dream / private cognition, Gate D / Envelope Audit implementation, R-surface, participation v2, writer authority.** Anchors: contract `docs/TORMENT_MODEL_API_TRUTHFULNESS_AUDIT_ADMISSIBLE_EVIDENCE_PACKET_CONTRACT_v0.1.md` (§2/§5 same-turn rule, §4 + §4A exclusions); boundary frame `444cc9b` (`docs/TORMENT_MODEL_API_TRUTHFULNESS_EVIDENCE_AUDIT_BOUNDARY_FRAME_v0.1.md`); Document B §7/§10 (B-O6/B-O6.1/B-O8); P4 §9/§10; Ledger Observational-Boundary §3; Track-A truthfulness envelope; MCP capability boundary. Claude proposes from source; GPT steers; Codex challenges boundary-bearing choices; Hilmir resolves true operator forks.
