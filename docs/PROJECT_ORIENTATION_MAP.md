...
- **HEAD:** `8e81557` / `origin/main` / clean.
...
- **Active next:** Hilmir has **selected the real-cognition direction** (priority order: (1) real cognition / meaningful Envelope Audit / dreaming; (2) participation guidance v2 later; (3) stronger anti-control locks later; (4) database/substrate last — already mapped/ratified, rechecked later). The **first doorway is FILED docs-only**: the **model-API truthfulness/evidence audit boundary frame** (`444cc9b`) — a bounded, **default-off, observe-and-surface-only**, ephemeral, **structurally non-reentrant, no-feedback** audit that observes evidence relation and reports coarse flags **which no runtime path honors**. **§9 operator confirmation is now given (Hilmir); the first step past the doorway is a docs-only non-reentry constraints lock (`bb9bb16`) — a pre-implementation constraints frame that authorizes no implementation/tests/schema/runtime/model-call/provider/endpoint/memory-write/output-control/substrate. Anything further is gated by that frame's closing gate: Hilmir explicit next-step confirmation + a Codex non-reentry challenge.** No model-API runtime / implementation / integration is opened; **dream remains downstream and separately gated** (Regime-B, not part of this doorway). The prior **A0 defer still stands** for the pre-substrate content-free shell (duplicates ReflectionTrace / `ReviewResult`, or risks the B-O6.1 anti-agreeableness failure); the model-API audit track is the chosen path to give real Envelope-Audit value. Current posture: **lane reopened past CLOSED-FOR-NOW into the C2 evidence-packet step.** The α empty-shell was **rejected** (duplicated ReflectionTrace / `/thinking/debug`); the smallest **real** evaluator boundary is a **Phase-8 after-the-fact sidecar** (`agent_loop.py` Phase 6–8) — **PASS on placement, coding still FAIL, with C2 (input minimization) the blocker**. Codex **accepted the evidence-packet direction WITH REQUIRED CORRECTIONS**: bounded **non-sensitive snippets from already-admitted response context only** (no fresh retrieval / no expanded context / no raw-hit rebuild), **limited evidence-relation only** (no full truthfulness verdict; full coverage stays §6-gated). Current motion: **docs-only Admissible Evidence Packet Contract** (`docs/TORMENT_MODEL_API_TRUTHFULNESS_AUDIT_ADMISSIBLE_EVIDENCE_PACKET_CONTRACT_v0.1.md`, commit pending); **next motion: tests-only packet-filter characterization**, then — only if green — a concrete implementation proposal. No code now; still authorizes **no** model call / provider / prompt / endpoint / persistence / output control / memory write / dream / Gate D / Envelope Audit / R-surface / participation v2 / writer authority / database/substrate. The lower-priority forks (participation v2; anti-control locks; database/substrate) remain unselected and later. Anchors: boundary frame `444cc9b` (`docs/TORMENT_MODEL_API_TRUTHFULNESS_EVIDENCE_AUDIT_BOUNDARY_FRAME_v0.1.md`); Document B §7/§10 (B-O6/B-O6.1/B-O8); P4 §9/§10; Ledger Observational-Boundary §3; Track-A truthfulness envelope; MCP capability boundary. Claude proposes from source; GPT steers; Codex challenges boundary-bearing choices; Hilmir resolves true operator forks.
