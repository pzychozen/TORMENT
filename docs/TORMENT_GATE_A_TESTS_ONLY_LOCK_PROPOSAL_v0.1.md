...
## 1. Status and scope
- HEAD = origin/main = `8e88d9c`. **Proposal only.** No tests, code, fixes, or locks authorized.
...
- **C5 may only** say the *current* Phase-7 route calls ordinary `fabric.ingest` with a compact summary
  and does **not** call any known Document-B / candidate / admission / promotion-force API **in that path**.
...
**Authorizable only as an AST/source guard.** Scope files: `thinking_controller.py`;
`geometric_harvester.py`; and `stance_policy.py` (where `determine_stance` lives) **only if** stance is
explicitly named in the final locked claim. **Do not include** `app.py`, `agent_loop.py`, `fabric.py`,
`spine.py`.
...
- *Must match on resolved call names, not raw substrings:* a raw text match must **not** flag substrings
  such as `"reingest"` (e.g., inside `collective_reingest`, comments, or unrelated identifiers) as a
  forbidden `ingest` call.
...
**Acceptable only as a current handler-shape lock.**
...
**Acceptable only as a direct-call absence lock.**
- *Assert:* `/agent/query` **may be tested for no direct** calls to `ingest`, `promote_chunk`,
  `promote_chunk_endpoint`, or `gravity_correction`; `/agent/query` calls `fabric.query`.
- *Must treat `fabric.query` as opaque and NOT mutation-free:* no direct advisory authority writer was
  found. This does **not** mean no mutation anywhere — `fabric.query` has retrieval-internal state
  effects.
- *Must explicitly say:* this does **not** assert `fabric.query` is mutation-free.
- *Test boundary (C3/C5):* C3/C5 must use fake or spy boundaries only; C3 must **not** assert real
  `fabric.query` purity.

## 6. Candidate lock C4 — MemoryPlan shape only
**Authorizable only as a shape/construction lock.**
- *Assert:* the lane booleans; `top_k_by_lane`; `weight_by_lane`; `safety_constraints`.
- *Do NOT test:* `fabric.query` clamp behavior; authority/admission influence; retrieval/query mutation
  behavior; lane behavior inside `fabric.query`.

## 7. Candidate lock C5 — Phase-7 ordinary-ingest routing characterization
**Authorizable only as a routing characterization with a fake-fabric spy.** Assert *current* behavior:
...
- this current Phase-7 route does **not** call any known Document-B / candidate / admission /
  promotion-force API in that path.
...
- *Test boundary (C3/C5):* C3/C5 must use fake or spy boundaries only; C5 must **not** characterize or
  bless real `fabric.ingest` downstream fan-out.

## 8. Rejected lock C6 — absence of Document B chamber / candidate store / dream runtime
**Do not test this absence.** C6 remains **rejected** because testing the absence of Document B /
chamber / dream / candidate store would freeze the absence of future governed implementation. It remains
docs-only characterization; a regression test on its absence would create a tripwire against future
