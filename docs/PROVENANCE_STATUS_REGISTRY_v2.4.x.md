...
| `WRITE_MIGRATION` | **Reserved (migration trio)** | Zero producers | Paired with `SOURCE_MEMORY` for legacy data migration tooling. Reserved by author intent; see §4 row for `SOURCE_MEMORY`. |
| `WRITE_SYSTEM_IMPORT` | **Reserved (migration trio)** | Zero producers | Paired with `SOURCE_MEMORY` for system-level imports (e.g. bootstrap data, seed memories). Reserved by author intent. |
