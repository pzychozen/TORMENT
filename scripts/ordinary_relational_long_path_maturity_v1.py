...
LABEL = "ordinary_relational_long_path_maturity_v1"
EXPERIMENT = "ORDINARY_RELATIONAL_LONG_PATH_MATURITY_V1"
TRAJECTORY_LABEL = "PREREGISTERED_EXPERIMENT_4_T1_FIRST_900"
TRAJECTORY_SCOPE = "PREREGISTERED_DISTINCT_EPISODE_RELATIONAL_MATURITY_TRAJECTORY"
EXPECTED_HEAD = "9dc346fff2f92ded5b3a72973e791e78ee103a8d"
EXPECTED_SUBJECT = "test(lived-use): preserve M6 short-root confirmation"
...
def load_trajectory() -> Dict[str, Any]:
    ...
    if not isinstance(rows, list) or len(rows) < MAX_EXCHANGES:
        raise StageStop(f"Preserved T1 has {len(rows) if isinstance(rows, list) else 'invalid'} rows; need {MAX_EXCHANGES}")
...
def short_root_for(timestamp: str) -> Path:
    ...
    return Path(r"C:\t") / f"n4b{compact[-10:]}"
...
def finalize_taxonomy(state: Mapping[str, Any], target_histories: Mapping[int, List[Mapping[str, Any]]]) -> Dict[str, Any]:
    ...
    weakening_labels = []
    for hist in target_histories.values():
        short_hist = [h for h in hist if h.get("post_route") == "short_path"]
        for prev, cur in zip(short_hist, short_hist[1:]):
            prev_score = prev.get("score")
            cur_score = cur.get("score")
            if isinstance(prev_score, (int, float)) and isinstance(cur_score, (int, float)):
                if float(cur_score) > float(prev_score):
                    weakening_labels.append("UP")
                elif float(cur_score) < float(prev_score):
                    weakening_labels.append("DOWN")
                else:
                    weakening_labels.append("SAME")
    if not weakening_labels:
        weakening = "NOT_DEMONSTRATED"
    elif all(v == "UP" for v in weakening_labels):
        weakening = "DEMONSTRATED"
    elif all(v == "DOWN" for v in weakening_labels):
        weakening = "CONTRADICTED"
    else:
        weakening = "MIXED"
    ...
    return {
        "SHORT_PATH_ROW_RECOMPRESSION": "POSSIBLE",
        "SHORT_PATH_REENTRY_OBSERVED": "DEMONSTRATED" if milestones.get("M4_FIRST_TARGET_RELATIONAL_REENTRY_AFTER_SHORT_PATH") else "NOT_DEMONSTRATED",
        "MULTIPLE_SHORT_PATH_PASSES_ON_SAME_RELATIONAL_EID": "DEMONSTRATED" if repeated else "NOT_DEMONSTRATED",
        "SHORT_PATH_WEAKENING_RAISES_LATER_RELATIONAL_SCORE": weakening,
        "REINFORCEMENT_CAN_AFFECT_COMPRESSED_ROW": "DEMONSTRATED" if state.get("compressed_row_reinforcement_observed") else "NOT_DEMONSTRATED",
        "RELATIONAL_COMPRESSION_AGE_SURVIVES_REINFORCEMENT": "DEMONSTRATED" if state.get("reinforced_target_age_survival_observed") else "NOT_DEMONSTRATED",
        "RELATIONAL_TIER_MIGRATION": "DEMONSTRATED" if state.get("relational_tier_migration_observed") else "NOT_OBSERVED",
        "RELATIONAL_ARCHIVE_MIGRATION": "DEMONSTRATED" if state.get("relational_archive_migration_observed") else "NOT_OBSERVED",
        "RELATIONAL_SCORE_0_7_MATHEMATICALLY_REACHABLE": "YES_CODE_TRACED",
        "RELATIONAL_SCORE_GE_0_7_BEFORE_AGE500": "DEMONSTRATED" if state.get("M6_BEFORE_AGE500") else "NOT_DEMONSTRATED",
        "AGE_500_GATE_REACHED_BY_ORDINARY_RELATIONAL_ROW": "DEMONSTRATED" if milestones.get("M7_FIRST_TARGET_RELATIONAL_AGE_GE_500") else "NOT_DEMONSTRATED",
        "POST_AGE500_AUTHENTIC_COMPRESSION_OPPORTUNITY": "DEMONSTRATED" if state.get("post_age500_opportunities") else "NOT_DEMONSTRATED",
        "RELATIONAL_SCORE_GE_0_7_AT_AGE_GE_500": (
            "DEMONSTRATED"
            if milestones.get("M8_FIRST_TARGET_RELATIONAL_SCORE_GE_0_7_AND_AGE_GE_500")
            else (
                "CONTRADICTED_IN_OBSERVED_EVENTS"
                if state.get("post_age500_opportunities") and not state.get("post_age500_score_ge_0_7_observed")
                else "NOT_DEMONSTRATED"
            )
        ),
        "ORDINARY_RELATIONAL_LONG_PATH": "DEMONSTRATED" if first_long else "NOT_DEMONSTRATED_IN_THIS_TRAJECTORY",
        "ORDINARY_RELATIONAL_LONG_PATH_ROUTE": "AGE_SCORE" if first_long else "NONE_OBSERVED",
        "FIRST_ORDINARY_RELATIONAL_LONG_PATH_STEP": (first_long or {}).get("step") if isinstance(first_long, Mapping) else "NONE",
        "FIRST_ORDINARY_RELATIONAL_DEEPMEMORY_STEP": (first_deep or {}).get("step") if isinstance(first_deep, Mapping) else "NONE",
        "CANDIDATE_CAP_MATERIAL_TO_RELATIONAL_SURVIVAL": state.get("candidate_cap_material_to_relational_survival", "NOT_ISOLATED"),
        "IDENTITY_ANCHOR_BEHAVIOR": "BACKGROUND_NOT_TARGET",
...
def run_experiment(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        for exchange, pair in enumerate(script, 1):
            ...
            if state.get("post_age500_opportunities") and len(state["post_age500_opportunities"]) >= 3:
                if first_target_long_path:
                    stop_reason = "FIRST_ORDINARY_RELATIONAL_LONG_PATH_OCCURRED"
                    break
                observed_ge_07 = bool(state.get("post_age500_score_ge_0_7_observed"))
                any_outside_ge_07 = False
                for event_record in compression_event_records:
                    for row in event_record.get("target_rows", []):
                        pre = row.get("pre") if isinstance(row.get("pre"), Mapping) else {}
                        diag = row.get("diagnostic") if isinstance(row.get("diagnostic"), Mapping) else {}
                        age = pre.get("age")
                        score = diag.get("candidate_score")
                        if isinstance(age, int) and age >= 500 and isinstance(score, (int, float)) and score >= 0.7:
                            if row.get("classification") == "ELIGIBLE_BUT_OUTSIDE_CANDIDATE_CAP":
                                any_outside_ge_07 = True
                if not observed_ge_07:
                    stop_reason = "THREE_POST_AGE500_COMPRESSION_OPPORTUNITIES_NO_RELATIONAL_THRESHOLD_CROSSING"
                    break
                if not any_outside_ge_07:
                    # If threshold-crossing rows were selected, an AGE_SCORE long path should have appeared.
                    stop_reason = "THREE_POST_AGE500_OPPORTUNITIES_WITH_THRESHOLD_CROSSING_NO_SUCCESS"
                    break

        if not stop_reason:
            if len(state.get("post_age500_opportunities", [])) < 3:
                stop_reason = "MAX_900_WITH_INSUFFICIENT_POST_AGE500_COMPRESSION_OPPORTUNITY"
            else:
                stop_reason = "MAX_900_REACHED_NO_ORDINARY_RELATIONAL_DEEPMEMORY"
